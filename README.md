...
> **收到新资料的固定动作（2026-05-13 起强制）**：每当用户、会议方或群里出现**任何**新信息（新的 docx / xlsx / 群消息 / 公众号链接 / 口头答疑等），都必须当场完成以下三步，缺一不可：
>
> 1. **存档**：原始文件落到 `_sources/` 对应子目录（已存在的目录优先），群消息/口头确认按"用户 `YYYY-MM-DD HH:MM` 在会话中说"原话引用即可，不必单建文件
> 2. **追加 SOURCES.md**：在对应 § 节追加一行/一段，标 🟢/🟡/⚫ 并指向 `_sources/` 内具体文件路径或时间锚
> 3. **同步 src/data 与组件**：把改动落到对应的 `src/data/*.ts` 数据层，必要时调整组件
>
> **SOURCES.md 只记最终源，不堆同会话改动 trail**：同一次对话里来回改稿、agent 自己理解错被纠正、措辞反复调整这些过程**不要**记进 SOURCES.md。这类条目会让以后 onboarding 的人误以为是有意义的修订史，实际只是噪音。每次修改 SOURCES.md 时直接覆盖到最终态即可。
>
> **真正需要保留为修订记录的，只限于跨会话/跨日的内容口径切换**——例如 docx 0512 替换早期版本、会议日程 v2 → v3、报告人题目从旧口径切到新 docx 等，这些是 onboarding 的人需要知道的"历史决策为什么是这样"，必须保留并标注时间锚。
>
...
- **引号**：`src/data/*.ts` 与 `src/components/*.tsx` 中**用户可见**的文案字符串里，引号按上下文选择：
  - 中文环境（前后是中文字符）→ 用中文弯引号 `"…"`（U+201C / U+201D），即使引号内夹着英文短语（如 `"量超智融合（QSIF, …）"`）也按中文环境处理
  - 纯英文环境（如英文 abstract 整段）→ 用 ASCII 直引号 `"…"`
- 收到新文案时按上述规则落盘；批量自检可用：
  ```bash
  grep -nP '"[^"]*[\x{4e00}-\x{9fff}][^"]*"' src/data/*.ts src/components/*.tsx
  ```
  正常应无输出（排除 `aria-label="…"` / `className="…"` 等纯英文属性，那是 JSX 语法不是文案）。
- 中文标点其它惯例：顿号 `、`、全角逗号 `，`、全角句号 `。`；英文术语前后**不**加空格除非夹在中文里时为了视觉间距（已存在的「中英文混排加空格」润色保留）。
